...
def first(request):
    ...
    return render(request, 'feed/first.html', context)
...
def posts_of_following_profiles(request):
    ...
    return render(request,'feed/feeds.html',{'profile':profile,'posts':posts_list})
...
def SaveView(request):
    ...
    if is_ajax(request=request):
        html = render_to_string('feed/save_section.html',context, request=request)
        return JsonResponse({'form':html})
...
def search(request):
    ...
    return render(request, 'feed/search_results.html', params)
...
def AllLikeView(request):
    user = request.user
    liked_posts = user.feedpost.all()
    context = {
        'liked_posts':liked_posts
    }
    return render(request, 'feed/liked_posts.html', context)
